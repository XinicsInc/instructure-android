...
import com.instructure.canvasapi2.models.post_models.CommentSendStatus
...
class SpeedGraderCommentHolder(view: View) : RecyclerView.ViewHolder(view) {
    companion object {
        val holderRes = R.layout.adapter_submission_comment
    }
    ...
    private fun CommentView.setDirection(commentDirection: CommentDirection) {
        ...
        when (commentDirection) {
            CommentDirection.INCOMING -> {
                setCommentBubbleColor(context.getColorCompat(R.color.comment_bubble_incoming))
                commentTextColor = context.getColorCompat(R.color.defaultTextDark)
            }
            CommentDirection.OUTGOING -> {
                setCommentBubbleColor(context.getColorCompat(R.color.comment_bubble_outgoing))
