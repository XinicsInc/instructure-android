...
class SettingsFragment : BasePresenterFragment<ProfileSettingsFragmentPresenter, ProfileSettingsFragmentView>(),
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        profileButton.onClick { RouteMatcher.route(context, Route(ProfileFragment::class.java, null)) }
        rateButton.onClick { RatingDialog.showRateDialog(activity, com.instructure.pandautils.utils.AppType.TEACHER) }
        legalButton.onClick { LegalDialog().show(fragmentManager, LegalDialog.TAG) }
        helpButton.onClick { HelpDialogStyled.show(activity) }
    }
