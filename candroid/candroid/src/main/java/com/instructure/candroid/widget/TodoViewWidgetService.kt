...
class TodoViewWidgetService : BaseRemoteViewsService(), Serializable {
    ...
    private inner class TodoViewsFactory(private val intent: Intent) : CanvasWidgetRowFactory<ToDo>() {
        ...
        override fun setViewData(streamItem: ToDo, row: RemoteViews) {
            ...
            if (streamItem.type == ToDo.Type.UPCOMING_EVENT) {
                row.setImageViewResource(R.id.icon, R.drawable.vd_calendar)
            } else if (streamItem.assignment.quizId > 0) {
                row.setImageViewResource(R.id.icon, R.drawable.vd_quiz)
            } else if (streamItem.assignment.discussionTopicHeader != null) {
                row.setImageViewResource(R.id.icon, R.drawable.vd_discussion)
            } else {
                row.setImageViewResource(R.id.icon, R.drawable.vd_assignment)
            }
        ...
        private fun getCourseCode(event: ToDo?): String? {
            return if (event == null || event.canvasContext == null || event.canvasContext.name == null) null
