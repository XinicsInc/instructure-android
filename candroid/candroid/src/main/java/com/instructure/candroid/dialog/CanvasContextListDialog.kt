...
class CanvasContextListDialog : AppCompatDialogFragment() {
    ...
    private fun updateCanvasContexts(courses: List<Course>, groups: List<Group>) {
        dialog.recyclerView.adapter = CanvasContextDialogAdapter(getCanvasContextList(context, courses, groups)) {
            dismiss()
            selectedCallback(it)
        }
    }
    ...
    private fun loadData(forceNetwork: Boolean) {
        apiCalls = tryWeave {
            dialog.emptyPandaView.setLoading()
    ...
    private fun getCanvasContextList(
        ...
        if (groups.isNotEmpty()) {
            ...
            groupSeparator.name = context.getString(R.string.groups)
            groupSeparator.id = -1
