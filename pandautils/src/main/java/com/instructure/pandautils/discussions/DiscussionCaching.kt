...
class DiscussionCaching(discussionTopicHeaderId: Long) {
    ...
    fun loadEntries(): MutableList<DiscussionEntry> {
        ...
        if(keys.isNotEmpty()) keys.forEachIndexed { _, key -> entries.add(book.read(key)) }
    ...
    fun exists(entry: DiscussionEntry): Boolean {
        return book.contains(entry.id.toString())
    }

    fun isEmpty(): Boolean {
        return book.allKeys.isEmpty()
    }
}
